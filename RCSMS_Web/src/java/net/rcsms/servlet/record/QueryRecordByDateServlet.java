...
public class QueryRecordByDateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sdate1.length() == 0){
            errorMessages.add("請輸入起始日期。 (年-月-日)");
        }else if(sdate1.length() == hyphencount1 || date1 == null){
            errorMessages.add("起始日期格式錯誤。 (年-月-日)");
        }else if(date1.after(Calendar.getInstance().getTime())){
            errorMessages.add("起始日期不得晚於今日。 (年-月-日)");
        }
        
        Date date2 = dateParse(sdate2);
        int hyphencount2 = sdate2.length() - sdate2.replace("-", "").length();
        if(sdate2.length() == 0){
            errorMessages.add("請輸入結束日期。 (年-月-日)");
        }else if(sdate2.length() == hyphencount2 || date2 == null || date2.after(Calendar.getInstance().getTime())){
            errorMessages.add("結束日期格式錯誤。 (年-月-日)");
        }else if(date2.after(Calendar.getInstance().getTime())){
            errorMessages.add("結束日期不得晚於今日。 (年-月-日)");
        }
        
        if(date1 != null & date2 != null && date1.after(date2)){
            errorMessages.add("起始日期不得晚於結束日期。 (年-月-日)");
        }
        
        if(errorMessages.isEmpty()){
            try{
                List<Record> recordlist = rjd.QueryByDate(date1, date2);
