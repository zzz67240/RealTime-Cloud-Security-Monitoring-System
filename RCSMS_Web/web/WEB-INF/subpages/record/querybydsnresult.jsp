...
<%
    ...
    out.println("<tr><th>序號</th><th>日期及時間</th><th>攝氏溫度</th>"
    ...
    if (recordlist != null) {
        for (Record record : recordlist) {
            out.println("<tr><td>" + record.getSerialnumber()+ "</td><td>" + record.getDatetime()
                    + "</td><td>" + record.getTemperature() + "</td><td>" + record.getHumidity() 
                    + "</td><td>" + record.getGas() + "</td><td>" + record.getSmoke() + "</td></tr>");
        }
        out.println("</table>");
    } 
%>
<br/>
<button id="smallbutton" onclick="window.location.href='/RCSMS_Web/record/QueryByDSN.jsp'">繼續查詢</button>
